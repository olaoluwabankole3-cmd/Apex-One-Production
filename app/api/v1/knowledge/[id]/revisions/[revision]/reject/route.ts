...
export async function POST(
  ...
  try {
    ...
    assertAllowedKeys(body, ["reason"] as const);
    const reason = Validator.requireString(body.reason, "reason", { minLength: 1, maxLength: 1000 });

    const result = await controlledKnowledgeService.rejectRevision(
      knowledgeId,
      revision,
      reason,
      ctx
