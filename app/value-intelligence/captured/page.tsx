...
export default function RecordedValueRoiPage() {
  const [selectedEvent, setSelectedEvent] = useState<any>(null);
  const [drawerOpen, setDrawerOpen] = useState(false);

  // Demo-only recorded value. Canonical verification/certification is not inferred from this amount.
  const recordedValueCaptured = 18400000; // ₦18.4M
  ...
  // Demo capture records with source/evidence descriptions. Record presence alone is not verification or certification.
  const capturedEvents: CapturedValueEvent[] = [
    ...
    },
    {
      ...
      auditTrail: [
        "Workload parameters checked across Lagos/Abuja teams",
        "Time tracking audit records 40 reclaimed advisor capacity hours"
      ]
    }
  ];

  // Demo visual Value Journey values (Identified -> Validated -> Executed -> Captured).
  const journeySteps = [
  ...
  const handleOpenDetail = (event: CapturedValueEvent) => {
    setSelectedEvent({
      ...
      whyIdentified: `This demo value event traces to a recorded APEX ONE audit source. Original potential target estimate was ${formatNaira(event.originalEstimate)}.`,
      evidence: event.evidenceDescription,
      recommendedAction: "Reconcile the recorded capture against authoritative financial records and canonical evidence history.",
      expectedOutcome: `Recorded ${formatNaira(event.capturedValue)} in captured yield with source traceability.`,
      executionStatus: "RECORDED CAPTURE",
      financialImpact: `+${formatNaira(event.capturedValue)} Recorded Ledger Yield`,
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 pb-12 select-none relative" id="recorded-value-workspace">
      ...
      <ValueHeader
        category="RECORDED VALUE / ROI EVIDENCE"
        title="RECORDED VALUE"
        subtitle="Track recorded financial outcomes and their source evidence. Canonical verification and certification are separate evidence states."
      />

      {!isDemoMode() ? (
        <ValueIntelligenceEmptyState
          title="No recorded value captured yet"
          description="Recorded financial returns, recovered revenue, and avoided costs will appear here once organizational data is connected and actions are executed."
        ...
        <>
            ...
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 relative z-10">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] font-mono text-emerald uppercase tracking-[0.2em] font-bold">Recorded Realized Capital</span>
                </div>

                <h1 className="font-display text-[46px] md:text-[60px] font-extrabold text-ivory tracking-tight leading-none font-mono">
                  {formatNaira(recordedValueCaptured)}
                </h1>
                <p className="mt-2.5 text-[12.5px] uppercase font-mono tracking-wider text-emerald font-bold">Recorded Value Captured</p>
                <p className="mt-3.5 text-[13px] text-ivory/50 max-w-xl leading-relaxed">
                  Demo captured-value records with source evidence. A displayed record is not automatically canonically verified or certified.
                </p>
              </div>
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-charcoal/40 p-5 shadow-glass space-y-4">
            <div className="flex items-center justify-between border-b border-white/[0.04] pb-2">
              ...
              <span className="text-[10px] font-mono text-emerald font-bold uppercase">Traceable Demo Records</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
            <div className="lg:col-span-8 space-y-4 text-left">
              <div className="flex items-center gap-2 px-1">
                ...
                <h4 className="text-[11px] font-mono font-bold text-emerald uppercase tracking-wider">CAPTURE EVIDENCE RECORDS</h4>
              </div>
                ...
                {capturedEvents.map((event) => (
                  ...
                  >
                    <div className="space-y-3">
                      ...
                      <div className="p-3 bg-white/[0.01] border border-white/[0.03] rounded-xl space-y-1">
                        <div className="flex items-center gap-1.5 text-emerald">
                          <Receipt size={12} />
                          <span className="text-[9.5px] uppercase tracking-wider font-mono block font-bold">Source Evidence ({event.evidenceType})</span>
                        </div>
                    ...
                    <div className="pt-2 border-t border-white/[0.03] flex items-center justify-between text-[11px] font-mono text-ivory/35">
                      <span>Ledger Code: recorded-capture</span>
            ...
            <div className="lg:col-span-4 space-y-4">
              <div className="flex items-center gap-2 px-1">
                ...
                <h4 className="text-[11px] font-mono font-bold text-emerald uppercase tracking-wider">APEX ROI RECORD</h4>
              </div>
              ...
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.01] p-6 shadow-glass space-y-6 text-left relative overflow-hidden">
                ...
                <div className="border-b border-white/[0.04] pb-3">
                  ...
                  <p className="text-[11px] text-ivory/40 mt-0.5">Demo trailing metrics for the recorded calendar cycle.</p>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center border-b border-white/[0.03] pb-2 text-[12.5px]">
                    ...
                    <span className="font-mono font-bold text-emerald">{formatNaira(recordedValueCaptured)}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-1">
                    <span className="text-[13px] text-emerald font-bold">MODELED RETURN:</span>
                ...
                <div className="p-4 bg-white/[0.01] border border-white/[0.04] rounded-xl text-[12px] text-ivory/65 leading-relaxed">
                  This demo dataset records an <strong className="text-emerald">8.8-to-1</strong> modeled cash-yield ratio. It is a presentation metric, not a canonical verification or certification decision.
                </div>

                <div className="pt-3 border-t border-white/[0.03] text-[10px] font-mono text-ivory/30 text-center uppercase tracking-wider">
                  Demo Ledger View: Recorded Evidence
                </div>
              </div>
            </div>
          </div>
