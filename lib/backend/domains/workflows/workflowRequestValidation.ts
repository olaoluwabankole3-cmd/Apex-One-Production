...
function parseConfiguration(
  ...
  for (const [key, rawValue] of entries) {
    ...
    throw new ValidationError(
      `Field '${fieldName}.${key}' must be a string, number, boolean, or string array`
    );
  }
...
function parsePosition(
  value: unknown,
  fieldName: string
): WorkflowNode["position"] | undefined {
...
export function parseCreateWorkflowRequest(
  body: JsonObject
): CreateWorkflowDto {
...
export function parseUpdateWorkflowRequest(
  body: JsonObject
): UpdateWorkflowDto {
  ...
  return {
    ...
    ...(body.subsidiary !== undefined
      ? {
          subsidiary: Validator.requireString(
            body.subsidiary,
            "subsidiary",
            { maxLength: 200 }
          ),
