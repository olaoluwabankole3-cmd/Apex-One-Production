...
export class RuleBasedDocumentExtractor implements IDocumentExtractor {
  public async extractFields(doc: DocumentRecord, content?: string): Promise<{
    ...
    if (doc.category === "Contract") {
      ...
      summary = `Analyzed ${doc.name}: Enterprise multi-year agreement. Includes quarterly pricing review stipulations and SLA escalation channels.`;
      ...
      summary = `Analyzed SLA agreement ${doc.name}. Defines tier-1 availability commitments and penalty credit thresholds.`;
